"""
Payroll journal entries for accounting systems.
Generates double-entry journal entries for each payroll run.

Standard payroll journal entry structure:
  DEBIT:  Wage Expense (gross wages)
  CREDIT: Payroll Tax Payable (employee taxes)
  CREDIT: Benefits Payable (pre-tax deductions)
  CREDIT: Garnishments Payable
  CREDIT: Cash / Accrued Payroll (net wages)

  DEBIT:  Payroll Tax Expense (employer taxes)
  CREDIT: Payroll Tax Payable (employer side)

GET /journal/{pay_run_id}        → journal entries JSON
GET /journal/{pay_run_id}/csv    → CSV download
GET /journal/{pay_run_id}/qbo    → QuickBooks IIF format
"""
import csv
import io
...
async def get_journal_entries(
    ...
    run, items, period, company = await _load(db, run_id, current_user["company_id"])
