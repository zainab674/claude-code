...
function useAuth() {
  ...
  const register = async (data) => {
    ...
    // On registration, we get a token back - technically we might need to fetch user info 
    // but the backend register returns just the token. Let's assume login after register.
    localStorage.setItem('payroll_token', res.access_token);
    const userRes = await api.getMe();
    localStorage.setItem('payroll_user', JSON.stringify(userRes));
    setUser(userRes);
  };
  const logout = () => {
...
// ─── Auth Pages ────────────────────────────────────────────
function LoginPage({ onLogin, onToggle }) {
  ...
  return (
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <p className="auth-sub">Welcome back. Enter your credentials to access your workspace.</p>
        ...
        <form onSubmit={submit}>
          ...
          <button className="btn btn-primary full-width" disabled={loading} style={{ marginTop: 12 }}>
            {loading ? 'Authenticating...' : 'Sign In to Dashboard'}
          </button>
        </form>
        <p style={{ marginTop: 24, fontSize: 12, color: 'rgba(255,255,255,0.4)', textAlign: 'center' }}>
          Demo Access: <span style={{ color: 'rgba(255,255,255,0.7)' }}>admin@acme.com / Admin123!</span>
        </p>
        <div style={{ marginTop: 32, textAlign: 'center', fontSize: 14, color: 'rgba(255,255,255,0.6)' }}>
          New to PayrollOS? <button className="btn btn-sm" onClick={onToggle} style={{ border: 'none', background: 'none', color: '#fff', fontWeight: 700, cursor: 'pointer', padding: '0 4px' }}>Create an account</button>
        </div>
      </div>
    </div>
  );
...
function SignupPage({ onSignup, onToggle }) {
      ...
      <div className="auth-card">
        ...
        <form onSubmit={submit}>
          ...
          <div className="form-group">
            ...
            <input type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="jane@acme.com" required />
          </div>
          <div className="form-group">
            <label>Password</label>
        ...
        <div style={{ marginTop: 32, textAlign: 'center', fontSize: 14, color: 'rgba(255,255,255,0.6)' }}>
          Already using PayrollOS? <button className="btn btn-sm" onClick={onToggle} style={{ border: 'none', background: 'none', color: '#fff', fontWeight: 700, cursor: 'pointer', padding: '0 4px' }}>Sign in instead</button>
        </div>
      </div>
    </div>
  );
...
function Employees() {
  ...
  const handleTerminate = async (emp) => {
    if (window.confirm(`Terminate ${emp.full_name}?`)) {
      await api.terminateEmployee(emp.id);
      load();
    }
  };
              ...
              {stubs.map(s => (
                <tr key={s.id}>
                  ...
                  <td>
                    ...
                    <a className="btn btn-sm" href={api.downloadPaystubUrl(s.id)} target="_blank" rel="noreferrer">PDF</a>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {selected && (
          <div className="card">
            <div className="card-header">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <a className="btn btn-primary" href={api.downloadPaystubUrl(selected.paystub_id)}
                  target="_blank" rel="noreferrer">↓ Download PDF</a>
                <button className="btn btn-sm" onClick={() => setSelected(null)}>✕</button>
              </div>
            </div>
            <div style={{ padding: 16 }}>
              <div style={{ marginBottom: 16, padding: 12, background: '#f8f8f8', borderRadius: 8, color: '#111' }}>
                <div style={{ fontWeight: 600, fontSize: 15 }}>
                  {selected.employee.first_name} {selected.employee.last_name}
                </div>
                <div style={{ color: '#333', fontSize: 12 }}>{selected.employee.job_title}</div>
              ...
              <div style={{ marginTop: 12, padding: 10, background: '#f0f8f0', borderRadius: 6, fontSize: 12, color: '#111' }}>
        ...
        {result && (
          <div className="card">
            ...
            <div style={{ padding: 16 }}>
              ...
              <div style={{ marginTop: 12, padding: 12, background: '#fff8e6', borderRadius: 8, fontSize: 12, color: '#111' }}>
                <strong>Employer true cost:</strong> {fmt(result.true_cost)}/period<br />
                <span style={{ color: '#444' }}>Includes employer FICA: {fmt(result.employer_total)}</span>
              </div>
            </div>
          </div>
        )}
...
export default function App() {
  ...
  if (!user) {
    return showSignup
      ? <SignupPage onSignup={register} onToggle={() => setShowSignup(false)} />
      : <LoginPage onLogin={login} onToggle={() => setShowSignup(true)} />;
  }
    ...
    <div className="app">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0 }}>
        <header style={{
          ...
          <div style={{ fontSize: 12, color: 'var(--text3)', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 12 }}>
            {user?.email}
            <button className="btn btn-sm" onClick={logout} style={{ padding: '2px 8px' }}>Sign out</button>
          </div>
        </header>
